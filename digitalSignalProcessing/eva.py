...
class MLP(chainer.Chain):
    ...
    def __init__(self, n_units, n_out):
        ...
        super(MLP, self).__init__(
            l1=L.Linear(None, n_units, initialW=w),  # n_in -> n_units    入力については，"None"にしておくと入力に応じて自動的にノード数を定義してくれる
...
def main():
    ...
    parser.add_argument('--inputimage', '-i', default='/home/pi/input.png',
                        help='入力画像ファイル')
    parser.add_argument('--model', '-m', default='./result/MLP.model',
                        help='DNNモデルで評価する')
    parser.add_argument('--unit', '-u', type=int, default=50,
                        help='Number of units')
    parser.add_argument('--out', '-o', type=int, default=2,
                        help='Number of final filter')
    args = parser.parse_args()

    # モデルのインスタンス作成
    model = L.Classifier(MLP(args.unit, args.out))
    # モデルの読み込み
